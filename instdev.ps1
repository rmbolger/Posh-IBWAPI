...
if ([String]::IsNullOrWhiteSpace($PSScriptRoot)) {
    ...
    $url = "https://github.com/rmbolger/Posh-IBWAPI/archive/$remoteBranch.zip"
    ...
    Remove-Item "$installpath\Posh-IBWAPI" -Recurse -Force -EA Ignore
    Write-Host "Renaming folder" -ForegroundColor Cyan
    Copy-Item "$installpath\Posh-IBWAPI-$remoteBranch\Posh-IBWAPI" $installpath -Recurse -Force -EA Continue
    Remove-Item "$installpath\Posh-IBWAPI-$remoteBranch" -Recurse -Force
    Import-Module -Name Posh-IBWAPI -Force
} else {
    # running locally
    Remove-Item "$installpath\Posh-IBWAPI" -Recurse -Force -EA Ignore
    Copy-Item "$PSScriptRoot\Posh-IBWAPI" $installpath -Recurse -Force -EA Continue
